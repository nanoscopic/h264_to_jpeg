...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
  ...
  temp=newTree->nil2= (rb_red_blk_node*) malloc(sizeof(rb_red_blk_node));
  temp->parent=temp->left=temp->right=temp;
  temp->red=0;
  temp->key=0;
  temp=newTree->root= (rb_red_blk_node*) malloc(sizeof(rb_red_blk_node));
  temp->parent=temp->left=temp->right=newTree->nil2;
  temp->key=0;
  temp->red=0;
  return(newTree);
}
...
void LeftRotate(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  rb_red_blk_node* y;
  rb_red_blk_node* nil2=tree->nil2;
  y=x->right;
  x->right=y->left;
  if (y->left != nil2) y->left->parent=x;
  y->parent=x->parent;   
  if( x == x->parent->left) {
    x->parent->left=y;
  } else {
...
void RightRotate(rb_red_blk_tree* tree, rb_red_blk_node* y) {
  ...
  x=y->left;
  y->left=x->right;
  if (nil2 != x->right)  x->right->parent=y; /*used to use sentinel here */
  x->parent=y->parent;
  if( y == y->parent->left) {
    y->parent->left=x;
  } else {
...
rb_red_blk_node* TreeSuccessor(rb_red_blk_tree* tree,rb_red_blk_node* x) { 
  ...
  if (nil2 != (y = x->right)) { /* assignment to y is intentional */
    while(y->left != nil2) { /* returns the minium of the right subtree of x */
      y=y->left;
    }
    return(y);
  } else {
    y=x->parent;
    while(x == y->right) { /* sentinel used instead of checking for nil2 */
      x=y;
      y=y->parent;
    }
    if (y == root) return(nil2);
    return(y);
  }
}
...
rb_red_blk_node* TreePredecessor(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (nil2 != (y = x->left)) { /* assignment to y is intentional */
    while(y->right != nil2) { /* returns the maximum of the left subtree of x */
      y=y->right;
    }
    return(y);
  } else {
...
void TreeForEach(rb_red_blk_tree *tree, void (*Func)(void*), rb_red_blk_node *x) {
    ...
    if( x->left != nil2 ) TreeForEach( tree, Func, x->left );
    Func( x->info );
    if( x->right != nil2 ) TreeForEach( tree, Func, x->right );
}
...
void TreeForEach1p(rb_red_blk_tree *tree, void (*Func)(void*,void*), void *param, rb_red_blk_node *x) {
    ...
    rb_red_blk_node* root = tree->root;
    if( !x ) x = tree->root->left;
    if( x->left != nil2 ) TreeForEach1p( tree, Func, param, x->left );
    Func( x->info, param );
    if( x->right != nil2 ) TreeForEach1p( tree, Func, param, x->right );
}
...
void RBDelete(rb_red_blk_tree* tree, rb_red_blk_node* z){
  ...
  y= ((z->left == nil2) || (z->right == nil2)) ? z : TreeSuccessor(tree,z);
  x= (y->left == nil2) ? y->right : y->left;
  ...
  if (y != z) { /* y should not be nil2 in this case */
